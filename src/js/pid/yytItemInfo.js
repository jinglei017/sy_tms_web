var app = new Vue({
    ...
    methods:{
        ...
        filesUpload(){
            ...
            $.ajaxFileUpload({
                url: pidUrl + '/import/EAItem?token='+that.logininf.token+'&timeStamp='+that.logininf.timeStamp, //用于文件上传的服务器端请求地址
                type: 'post',
        ...
        getOrderDetails(title,pdItem,type){
            ...
            getRequest(pidUrl + "/get/eaItemDetails.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdItemId="+pdItemId+"&itemCode="+itemCode,function(res){
                if(res.result != null){
                    if(res.result.itemInfo != null){
                        ...
                        if(res.result.itemInfo != null){
                            that.itemSingle0 = res.result.itemInfo;
                        }
        ...
        changeOrderDetails(title,pdItem){
            ...
            that.isDisable = false;
            getRequest(pidUrl + "/get/eaItemDetails.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdItemId="+pdItem.pdItemId+"&itemCode="+pdItem.itemCode,function(res){
                if(res.result != null){
                    if(res.result.itemInfo != null){
                        if(res.result.itemInfo.item != null){
                            that.itemSingle = res.result.itemInfo.item;
        ...
        editOrderInfo(){ //保存修改信息-基本信息 （暂时不改商品属性信息）
            ...
            if(that.itemSingle0.prackingUnit == "" || that.itemSingle0.prackingUnit == undefined){
                imitatePopup("包装单位不能为空",'alert');
                return false;
            }
            if(that.itemSingle0.prackingSpec == "" || that.itemSingle0.prackingSpec == undefined){
                imitatePopup("包装规格不能为空",'alert');
                return false;
            }
            if(that.itemSingle0.taxRate == "" || that.itemSingle0.taxRate == undefined){
                imitatePopup("税率不能为空",'alert');
                return false;
            }
            if(that.itemSingle0.brand == "" || that.itemSingle0.brand == undefined){
                imitatePopup("品牌不能为空",'alert');
                return false;
            }
            ...
            paramsObj.prackingSpec = that.itemSingle0.prackingSpec;
            paramsObj.taxRate = that.itemSingle0.taxRate;
            paramsObj.brand = that.itemSingle0.brand;
            paramsObj.prackingUnit = that.itemSingle0.prackingUnit;
            paramsObj.partyCode = that.itemSingle0.partyCode;
            paramsObj.partyName = that.itemSingle0.partyName;
            //保存修改信息
            postRequest(pidUrl + "/update/pdEAItem.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
                    // 关闭侧滑 ------ start
                closeSideslip();
                    var searchFormArr = {
                        pageInfo:{
        ...
        saveNewOrderInfo(){
            ...
            paramsObj.partyName = that.itemSingle0.partyName;
            //保存新增信息
            postRequest(pidUrl + "/add/eaItemInfo.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
