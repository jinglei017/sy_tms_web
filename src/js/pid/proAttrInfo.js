var app = new Vue({
    ...
    methods:{
        ...
        getOrderDetails(title,pdItem){
            ...
            that.isDisable = true;
            getRequest(pidUrl + "/get/pdSpec.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdSpecId="+pdItem.pdSpecId,function(res){
                if(res.result != null){
        ...
        deleteOrderDetails(pdItem){
            var that = this;
            var r = confirm("确定删除该产品属性");
            if (r==true){
                getRequest(pidUrl + "/delete/pdSpec.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdSpecId="+pdItem.pdSpecId,function(res){
                    //获取订单列表
                    var searchFormArr = {
                        pageInfo:{
                    ...
                    postRequest(pidUrl + "/select/pageSpecBySpecVo.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,searchFormArr,function(res){
        ...
        getSearchVal(){ //搜索
            var that = this;
            var searchFormArr = this.queryParam;
            if(searchFormArr.specCode == ''){
                searchFormArr.specCode = undefined;
            }
        ...
        editOrderInfo(){
            var that = this;
            if(that.itemSingle.specCode == "" || that.itemSingle.specCode == undefined){
                imitatePopup("产品属性代码不能为空",'alert');
                return false;
            }
            if(that.itemSingle.specLabel == "" || that.itemSingle.specLabel == undefined){
                imitatePopup("产品属性名称不能为空",'alert');
                return false;
            }
            var paramsObj = that.itemSingle;
            //保存修改信息
            postRequest(pidUrl + "/update/pdSpec.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
        ...
        saveNewOrderInfo(){
            ...
            var paramsObj = that.itemSingle;
            //保存新增信息
            postRequest(pidUrl + "/insert/pdSpec.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
