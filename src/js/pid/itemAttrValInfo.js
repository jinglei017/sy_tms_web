var app = new Vue({
    ...
    methods:{
        ...
        getOrderDetails(title,pdItem){
            ...
            that.isDisable = true;
            getRequest(pidUrl + "/get/pdPropValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdPropValueId="+pdItem.pdPropValueId,function(res){
                if(res.result != null){
        ...
        deleteOrderDetails(pdItem){
            var that = this;
            var r = confirm("确定删除该商品属性值");
            if (r==true){
                getRequest(pidUrl + "/delete/pdPropValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdPropValueId="+pdItem.pdPropValueId,function(res){
                    //获取订单列表
                    var searchFormArr = {
                        pageInfo:{
                    ...
                    postRequest(pidUrl + "/select/pagePropValueByPropValueVo.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,searchFormArr,function(res){
        ...
        editOrderInfo(){
            var that = this;
            if(that.itemSingle.propValue == "" || that.itemSingle.propValue == undefined){
                imitatePopup("商品属性值不能为空",'alert');
                return false;
            }
            var paramsObj = that.itemSingle;
            //保存修改信息
            postRequest(pidUrl + "/update/pdPropValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
                    // 关闭侧滑 ------ start
                closeSideslip();
                    var searchFormArr = {
                        pageInfo:{
        ...
        saveNewOrderInfo(){
            ...
            var paramsObj = that.itemSingle;
            //保存新增信息
            postRequest(pidUrl + "/insert/pdPropValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
