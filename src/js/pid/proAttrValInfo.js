var app = new Vue({
    ...
    methods:{
        ...
        getOrderDetails(title,pdItem){
            ...
            that.isDisable = true;
            getRequest(pidUrl + "/get/pdSpecValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdSpecValueId="+pdItem.pdSpecValueId,function(res){
                if(res.result != null){
        ...
        deleteOrderDetails(pdItem){
            var that = this;
            var r = confirm("确定删除该产品属性值");
            if (r==true){
                getRequest(pidUrl + "/delete/pdSpecValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp+"&pdSpecValueId="+pdItem.pdSpecValueId,function(res){
                    //获取订单列表
                    var searchFormArr = {
                        pageInfo:{
                    ...
                    postRequest(pidUrl + "/select/pageSpecValueBySpecValueVo.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,searchFormArr,function(res){
        ...
        editOrderInfo(){
            var that = this;
            if(that.itemSingle.specValue == "" || that.itemSingle.specValue == undefined){
                imitatePopup("产品属性值不能为空",'alert');
                return false;
            }
            var paramsObj = that.itemSingle;
            //保存修改信息
            postRequest(pidUrl + "/update/pdSpecValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
        ...
        saveNewOrderInfo(){
            ...
            var paramsObj = that.itemSingle;
            //保存新增信息
            postRequest(pidUrl + "/insert/pdSpecValue.json?token="+that.logininf.token+"&timeStamp="+that.logininf.timeStamp,paramsObj,function(res){
