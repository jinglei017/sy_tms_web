var app = new Vue({
    ...
    methods:{
        ...
        addOrderDetails(template){
            ...
            this.orderDetail.completionTime = getTodayTime();
            this.orderDetail.expireTime = getTodayTime();
        ...
        saveNewOrderInfo(){ // 订单  新增、修改
            ...
            if(that.orderDetail.expireTime == ""||that.orderDetail.expireTime == undefined){
                alert("请选择订单过期时间");
                return false;
            }
            if(that.orderDetail.totalQty == "" && that.orderDetail.totalQty != 0){
                alert("请填写总数量");
                return false;
            }
            if(that.orderDetail.totalWeight == "" && that.orderDetail.totalWeight != 0){
                alert("请填写总重量");
                return false;
            }
            if(that.orderDetail.totalVolume == "" && that.orderDetail.totalVolume != 0){
                alert("请填写总体积");
                return false;
            }
            if(that.orderDetail.totalAmount == "" && that.orderDetail.totalAmount != 0){
                alert("请填写总金额");
                return false;
            }
            ...
            /*if(that.orderDetail.orderType == 'DO' ){
                omOrderHeadInfoModel.carrierPartyInfo = OrderCarrier;
            }else{
                omOrderHeadInfoModel.carrierPartyInfo = {};
            }*/
        ...
        addPackageGoods(){ // 添加包裹商品
            ...
            that.packageGoodsList.push({
                orderItem:{
                    ...
                    lotNo:"",
                    unitPrice:"",
                    currency:that.selectListData.currencyList[0].code,
                    qty:"",
                    qtyUnit:that.selectListData.qtyUnitList[0].code,
                    weight:"",
                    weightUnit:that.selectListData.weightUnitList[0].code,
                    volume:"",
                    volumeUnit:that.selectListData.volumeUnitList[0].code,
                    amount:""
                }
            });
        },
        ...
        getSearchVal1(){ // 查询 - 合作商
            var that = this;
            var params = that.queryParam1;
            if(that.queryParam1.partyType == "null"){
                that.queryParam1.partyType = null
            }
            params.pageInfo = {
                pageNum:1,
                pageSize:20
...
$(document).ready(function() {
    ...
    $('input[name="completionTime"]').daterangepicker({ singleDatePicker: true }, function(start, end, label) {
        app.orderDetail.completeTime= start.format('YYYY-MM-DD');
    });

    $('input[name="expireTime"]').daterangepicker({ singleDatePicker: true }, function(start, end, label) {
        app.orderDetail.expireTime= start.format('YYYY-MM-DD');
    });
});
